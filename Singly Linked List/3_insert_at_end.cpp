 ...
 * 
 * Time Complexity = O(N); N = Length of the SLL
 * Auxiliary Space = O(1)
*/
void insert_at_end(Node** head_ref, int new_data) {
    ...
    // 3. New node's next should be NULL
    new_node->next = NULL;

    // 4. Check if the head is NULL, make the new_node as first
    if(*head_ref == NULL) {
        *head_ref = new_node;
        return;
    }

    // 5. Else go to the end of the SLL, traverse till the node's next is NULL
    Node* last = *head_ref;
    while(last->next != NULL) {
        last = last->next;
    }
    
    // 6. Last node's next should point to new_node
    last->next = new_node;
}
